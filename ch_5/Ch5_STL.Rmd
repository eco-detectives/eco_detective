...
for(i in 1:length(X)){
  ...
  Ypredic[i]<- A+(B*X[i])+(C*((X[i])^2))
}

UncertainPredictions <- data.frame(Ypredic, YpredicUnc) %>%
  gather(key="model", value="predY") %>%
  mutate(X= rep(X, length.out=nrow(UncertainPredictions)))
