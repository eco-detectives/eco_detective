...
Bycatch <- vector(length=length(Uvec))
for (j in 1:nsim){
...
#CumPdf$c[which(Uvec[i] < CumPdf$CumP & Uvec[i] > CumPdf$CumPFloor)]
...
```{r eval=FALSE}
SimCatch <- matrix(NA, nrow=Ntow, ncol=nsims)
U <- vector(length=length(Ntow))
C <- vector(length=length(Ntow))
for (j in 1:nsims){
for(i in TowVec){
  U[i] <- runif(n=1,min=0,max=1)
   #joins Uvec and CumPDF and finds the nearest value, stores catch in c
}
U <- as.data.frame(U)
x <- CumPdf[U, roll="nearest"]
SimCatch[,j] <- x$c
return(SimCatch)
}
